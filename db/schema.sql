...
-- stopped scaling once the catalog passed ~1M rows (observed: single
...
-- an old timestamp (observed for mast.py). reconcile.py periodically
...
CREATE TABLE spectroscopy_holdings (
    ...
    -- (real ObsCore calib_level, observed 2026-08-03), koa (raw-only
    ...
    -- observed 2026-08-04 against each module's own deep-link/
...
INSERT INTO archives (archive_code, display_name, access_mechanism, has_native_gaia_column, native_gaia_dr, notes) VALUES
    ...
    ('gemini_ghost',        'Gemini Observatory Archive — GHOST', 'rest_json', FALSE, NULL, 'GHOST-specific: CADC is missing GHOST''s reduced per-arm spectra (observed), so this goes straight to GOA (archive.gemini.edu) instead. Needs an authenticated session cookie — see sync/archives/gemini_ghost.py.'),
    ('gemini_igrins',       'Gemini Observatory Archive — IGRINS', 'rest_json', FALSE, NULL, 'IGRINS-specific: CADC has zero reduced IGRINS planes at all (observed), so this goes straight to GOA instead, same authenticated pattern as gemini_ghost — see sync/archives/gemini_igrins.py.'),
    ('mast',                'MAST',                             'tap',       FALSE, NULL, 'Implemented against HST, IUE, FUSE, EUVE, HUT, TUES, BEFS, WUPPE via mast.stsci.edu/vo-tap (ivoa.obscore) — a different, working TAP service, not the classic API. IUE/FUSE/EUVE/HUT/TUES/BEFS/WUPPE all need access_format=''image/fits'' (not HST''s ''application/fits'') and a client-side dedup by obs_id (many rows per obs_id there, picks the _vo.fits canonical product) — see sync/archives/mast.py. The 5 UV rocket/shuttle missions beyond HST/IUE/FUSE were added later, found via a VO SSA-registry sweep, and are a same-day extension of this same query (no new endpoint/shape) — observed, 9,830/8,726/4,678/2,719/1,429 rows respectively. GALEX deliberately excluded despite 1.56M real spectrum rows existing here too -- slitless-grism spectra in often-crowded fields, needs a data-quality pass first. No cliff found. JWST is a separate archive_code (mast_jwst) — its 504 on this same query shape turned out to need bounded-window pagination, not a dead end.'),
    ('mast_jwst',           'MAST — JWST',                      'tap',       FALSE, NULL, 'Same TAP service as mast, split out because JWST needs bounded MJD-window pagination (an unbounded watermark query 504s for this collection specifically, even at TOP 10 — a genuine server-side cliff, not a row-cap issue) and a per-obs_id product-suffix ranking (_x1d/_s3d/_c1d preferred over intermediate cal/rate/s2d products and unrelated guide-star calibration rows that share the same obs_id) instead of mast.py''s single-suffix _vo.fits dedup — see sync/archives/mast_jwst.py.'),
    ...
    ('sdss_v_optical',       'SDSS-V — Optical',                 'bulk_file', TRUE,  'dr3', 'Implemented directly against the bulk spAll-lite file (now DR20, ~2.5GB gzip) — GAIA_ID 100% populated for CLASS=STAR in the DR19 sample, including live-confirmed FPS-era rows. DR20 (shipped 2026-07-31) observed via the FITS header that GAIA_ID switched from Gaia DR2 to DR3 source_id, as expected.'),
    ('sdss_legacy_optical',  'SDSS Legacy Optical',              'cas_sql',   FALSE, NULL, 'Implemented — no Gaia column, positional match via specObj ra/dec, capped at MJD 58932. archive_url points at a real direct spec FITS file (sdss/redux/{run2d}/spectra/lite/{plate}/spec-{plate}-{mjd}-{fiberid}.fits, confirmed live) — previously a SkyServer object-explorer page, not a data file. See sync/archives/sdss_legacy_optical.py. Pre-BOSS rows (SDSS-I/II, mjd < 55176) are orphaned leftovers from an abandoned first-cut sync and permanently outside that module''s query scope — DR20 has no per-object file for them at all, only a per-plate spPlate-{plate}-{mjd}.fits; scripts/backfill_sdss_legacy_pre_boss.py repoints those rows'' archive_url at the right spPlate file + fiber number, and webapp/spectrum_viewer.py''s _parse_sdss_legacy_plate range-fetches just that one fiber''s row.'),
    ...
    ('elodie',                  'ELODIE (OHP)',                      'html_form', FALSE, NULL, 'Implemented against atlas.obs-hp.fr/elodie (plain HTTP, no TLS) -- no per-object query needed, the CGI returns the entire decommissioned archive (35,535 rows total, observed) as one fixed-width plain-text table when no object filter is given. Half the archive (19,289 rows) is Th-Ar calibration frames, filtered out via an imatyp prefix check, leaving 16,246 real science spectra. Fixed-width parsing required -- naive whitespace-splitting misparses rows with a blank objname or corrupt coordinate field (observed, both occur). Both name and position available (packed J2000 string parsed into ra/dec) -- normal identifier-then-position matching, unlike feros_gavo/flashheros_gavo. Final, decommissioned instrument (last observed 2006) -- one-shot pull.'),
    ('sophie',                  'SOPHIE (OHP)',                      'html_form', FALSE, NULL, 'Implemented against the same OHP host/CGI engine as elodie, but this table has no blank/wildcard bulk dump (observed: both an empty object filter and a bare "%" wildcard return 0 rows) -- paginated instead by iterating a fixed list of common stellar catalog prefixes (HD%, BD%, TYC%, HIP%, GJ%, 2MASS%, Gaia DR%), the archive''s own documented technique for pulling a broad group. Real but incomplete coverage: HD% alone returns 67,714 of the archive''s ~104,105 total rows -- a star cross-matched under a name outside this prefix list will be missed. Same fixed-width parsing and packed-J2000-coordinate shape as elodie. Querying by star name implicitly excludes calibration frames (no separate type filter needed, unlike elodie''s imatyp check).'),
    ('salt_hrs',                'SALT HRS (SAAO SSDA)',              'graphql',   FALSE, NULL, 'Implemented against ssda.saao.ac.za/api -- a GraphQL API, not TAP/VO (the site''s own /tap and /vo/tap routes are just SPA catch-all paths, not real endpoints). Query shape reverse-engineered from the SPA''s own JS bundle -- the `where` arg is a String that must contain a specific JSON filter shape (e.g. {"EQUALS":{"column":"instrument.name","value":"HRS"}}), and `columns` needs exact dotted table.column paths, both only discoverable by grepping the bundle. 47,495 HRS rows observed. No position data at all (confirmed) -- name-only match, same as feros_gavo/flashheros_gavo. Paginated via a GREATER_EQUAL watermark on observation_time.start_time (epoch ms) rather than startIndex alone, since startIndex-only pagination would never notice new observations added after a prior run''s cursor reached the end. Includes embargoed/not-yet-public rows (confirmed the archive lists them) -- archive_url will 403 until each file''s own data_release date passes, same as any other archive''s proprietary content.'),
    ('ing',                     'ING Archive (WHT/ISIS)',            'html_form', FALSE, NULL, 'Implemented against casu.ast.cam.ac.uk/casuadc/ingarch (the old archive.ast.cam.ac.uk is dead) -- a TurboGears web form, no API. Metadata-only by design: bulk file retrieval only exists via a stateful, email-gated async job queue with no way to poll for completion (observed) -- since every archive_url elsewhere in this project already just points at the source archive rather than downloading bytes, archive_url here points at displayHeader?recno=... instead, a real directly-fetchable page, same role the portal link plays for lbt.py. WHT/ISIS only (server-side instrument=ISIS filter, confirmed real substring filtering) -- WHT/ACAM and WHT/LIRIS are dual imaging/spectroscopy instruments with no mode field in the default columns to tell which is which, deliberately excluded. obs_type=TARGET filters out ARC/BIAS/FLAT/SKY calibration. No offset/watermark field exists at all -- paginated via an adaptive nightobs calendar-window walk (bisects on the archive''s own undocumented 1000-row display cap, grows back up after a successful pull) since window size needed varies hugely across ING''s ~40-year history.'),
    ('naoj',                    'NAOJ (Subaru HDS, via JVO)',        'tap',       FALSE, NULL, 'Not the SMOKA archive (still a dead end: registration-gated web wizard, no bulk API) -- implemented against a separate TAP+SSA service run by JVO (jvo.nao.ac.jp/skynode/do/tap/hds/sync) for Subaru''s High Dispersion Spectrograph, found via the reg.g-vo.org registry sweep. A custom JVOQL engine, not DaCHS -- SELECT * is unusable (a malformed access_estsize column declared int but emitting decimal strings crashes the VOTable parser, observed), no instrument_name column (hardcoded to HDS, the table''s only instrument), COUNT(DISTINCT ...) silently ignored (observed), 200,000-row server-side cap regardless of TOP/maxrec. 253,389 rows observed, most raw_ids carrying several pipeline-product rows of the same exposure (fits + text/plain variants) -- deduped per-page by a product-rank preferring the fully-processed 1D fits product, same shape as mast_jwst.py''s per-obs_id ranking. Target name and wavelength range are packed together in obs_title ("NAME [lo:hi]"), parsed apart. No cliff found in TOP+ORDER BY t_mid pagination.'),
    ('neid',                    'NEID (WIYN, Kitt Peak)',           'tap',       FALSE, NULL, 'Implemented via a real, no-auth TAP service at neid.ipac.caltech.edu/TAP/sync (observed via TAP_SCHEMA.tables), covering neidl2 -- the pipeline''s final wavelength-calibrated, RV-ready extracted-spectrum tier (datalvl=2 on literally every row) -- not neidl1. Solar variants (neidsolarl1/neidsolarl2, NEID''s dedicated solar feed) excluded -- the Sun, not a star. Backed by Oracle, not the DaCHS/PostgreSQL engine behind most other TAP archives here (observed via a literal ORA-00937 error from a malformed test query). 21,536 rows observed, 987 distinct real target names topped by HD 10700 (Tau Ceti, 1,062 obs) and other well-known bright RV-survey stars. No cliff found in TOP+ORDER BY obsjd pagination (obsjd observed to have 0 nulls and 0 duplicate values across the whole table, unlike xmm.py/naoj.py no tie-handling is needed). No ObsCore access_url-equivalent column and no discoverable per-exposure deep link on the archive''s own Firefly-based search.php frontend (guessed direct-file endpoints observed 404) -- archive_url points at that general search portal instead, same convention as lbt.py.'),
    ('not_fies',                'NOT (Nordic Optical Telescope) — FIES', 'html_form', FALSE, NULL, 'Implemented against www.not.iac.es/observing/forms/fitsarchive (index.php/query.php) -- a bespoke FITS-header archive, no TAP/API, covering FIES only (the same form also covers ALFOSC/MOSCA/NOTCam/StanCam, NOT''s imagers, excluded here). query.php itself flatly rejects requests without a real User-Agent/Referer (observed, same bot-blocking shape as gtc.py); a separate, much weaker gate on show.php (the per-file link each result row carries) accepts any Referer merely containing the substring "query.php", observed even off-domain -- since a real user would never send that by accident, archive_url instead points at index.php?instrument=FIES&name=<target>, a real always-working page that reflects the target name into the form. criteria=wholesky with an IMAGETYP=''OBJECT'' filter isolates real science exposures from calibration frames (a small number of "FIEStool flat" frames still slip through unfiltered, same as gtc.py''s own unflitered free-text names). No offset/limit field and a real silent hard cap of exactly 1000 rows (observed) -- paginated via an ing.py/gtc.py-style adaptive DATE-OBS calendar-window walk. Real 12-month proprietary period observed to apply even to OBJECT-filtered rows. Row HTML has each result''s <a> left unclosed until the row''s last </td> (observed) -- parsed via regex rather than a DOM parser, same workaround as ing.py''s own malformed table.'),
    ('oirsa',                   'OIRSA (CfA)',                      'tap',       FALSE, NULL, 'Implemented against a real TAP service at oirsa.cfa.harvard.edu:8080/tap (found via the reg.g-vo.org registry sweep) -- the archive''s own :443 web frontend is a stateful dojo/prototype.js search app with no scriptable API (observed: its /search/* AJAX endpoints 404 for any non-browser client), entirely unrelated to this TAP service. ivoa.obscore unifies all four CfA instruments: FAST (132,452 rows), Hectospec (599,592), Hectochelle (393,267), Echelle (171,278) -- ~1.3M spectra observed, pulled unfiltered since obs_collection is only populated for Echelle (not usable as a discriminator) -- instrument_name read per-row instead just to label each observation. Hectospec/Hectochelle target_name is a plate/configuration id, not a star name, but s_ra/s_dec are still genuine per-fiber target positions (observed: rows sharing one target_name carry different positions, s_fov ~1.5 arcsec) -- positional matching still works even though names don''t resolve. access_url is already a direct per-row file link, no DataLink resolution needed. No cliff found in TOP+ORDER BY t_min pagination up to 50,000 rows/page, same shape as dao.py.'),
    ('gtc',                     'GTC (Gran Telescopio CANARIAS)',    'html_form', FALSE, NULL, 'A bare GET on searchform.jsp reproducibly 500s; this is the JSP app rejecting requests with no session cookie/User-Agent, not a real server bug. Once past that, the actual search (searchres.jsp, POST multipart/form-data) needs no session at all. instCode scopes to spectroscopy-only modes (OSI_LSS, OSI_MOS, MEG_SPE, MEG_IFU, HORuS_SPE, CC_SPE, EMIR_SPE), excluding imaging/polarimetry. 719,927+ products observed. Each row''s Program ID + OBlock ID + numeric ProdId composite key drives FetchProd, a real ungated per-exposure raw-FITS download servlet (no login) -- built by hand for embargoed rows (whose HTML omits the link but still shows the three components), same 403-until-release convention as salt_hrs.py. rpp (page size) accepts values well past the form''s advertised 10/50/100 max but has a real cliff past ~5,000-10,000 rows/page. Default order is newest-first on a live, actively-growing archive (the "N products found" count has been observed to change between two back-to-back requests) -- a page-number/frontier cursor would silently miss new inserts, so this paginates via an adaptive calendar-window walk on the date fields instead (same shape as ing.py), which is immune to that drift.'),
    ('hermes_mercator',         'HERMES (Mercator Telescope, KU Leuven)', 'tap',  FALSE, NULL, 'Added from a user-supplied web form (mercatorvo.ster.kuleuven.be/hermes/q/web/form) -- same GAVO DaCHS/SSA software as feros_gavo/flashheros_gavo (identical ssa_* fields), and like every DaCHS service it also exposes a real TAP endpoint (mercatorvo.ster.kuleuven.be/tap, table hermes.data) instead of needing to scrape the form. Unlike feros_gavo/flashheros_gavo, ssa_location is populated on every one of 119,650 rows observed -- normal identifier-then-position matching applies, not name-only. ssa_instrument is a constant literal "HERMES ()" on every row (hardcoded to "HERMES" here instead). embargo column is always blank -- not usable as an embargo signal, unlike harpsn_tng''s policy field. Paginated via a unique_seqno id watermark (same shape as harpsn_tng/asiago) since the archive is still actively growing, though no cliff was found at this archive''s much smaller scale.'),
    ('bess',                    'BeSS (Be Star Spectra, Observatoire de Paris/OHP)', 'html_form', FALSE, NULL, 'Previously written off as blocked on a real bug (the FITS download button returns HTTP 200 with correct headers but a 0-byte body, observed) -- irrelevant once discovered that a per-spectrum plot PNG at a predictable static path (Spectres_png/S{id:07d}[:3]/sp_{id:07d}.png) is public with no session needed, unlike everything else on this ~20-year-old pure-PHP site, which needs a bootstrap session (Accueil.php then MenuIntro.php) plus a plausible Referer on every request. Consul.php''s own POST search form is a dead end (always returns zero results regardless of input, confirmed with a real resolved star) -- the working path is StarConsul.php (lists all 1506 Be stars that have >=1 spectrum) then Consul.php?specobj=<name> per star. Has real ra/dec (sexagesimal) and target name -- normal identifier-then-position matching applies. No incremental watermark exists (Consul.php''s own pagination cursor is opaque, confirmed non-numeric-offset) -- fetch() does a one-shot full crawl of all 1506 stars, same "static archive, cursor short-circuits to no-op once finished" pattern as rave/galah/elodie/sophie/salt_hrs; a fresh crawl to pick up new amateur uploads needs the cursor cleared by hand. reduction_status is hardcoded ''reduced'' -- every submission is a wavelength-calibrated 1D extracted spectrum, checked against a required FITS format at upload, never a raw CCD frame, though BeSS''s own FAQ notes flux specifically is not calibrated.'),
    ('hpol',                    'HPOL (Wisconsin H-alpha/HPOL spectropolarimeter, STScI)', 'html_form', FALSE, NULL, 'Not present in mast.py''s VO-TAP obscore table alongside its siblings IUE/WUPPE/EUVE/... there (observed: obs_collection=''HPOL'' returns 0 rows) -- needs its own scraper against the legacy archive.stsci.edu/hpol/search.php form instead, no TAP/API. An empty-filter POST with outputformat=CSV and max_records past the true total returns the entire decommissioned (2004) archive in one response -- observed, 4,836 rows, matching the form''s own "N are available" banner. Real ra/dec (space-separated sexagesimal) and target name on every row -- normal identifier-then-position matching applies. Download filenames are NOT a single deterministic pattern: HPOL''s detector was upgraded from Reticon to CCD partway through the mission, and the real prefix (hpolret_ vs hpolccd_) is observed to NOT be derivable from the Data ID''s shape or date alone (the two eras genuinely overlap, 1991-1992 has both, and same-shaped ids like the "b"/"r" channel-split suffix appear under both prefixes) -- resolved instead via each id''s own Apache directory listing at missions/hpol/data/{id}/, one extra GET per record, so fetch() batches this in bounded per-call chunks (200 records/call) even though the metadata pull itself is one-shot. reduction_status left unset -- no calib_level-equivalent column and no established fact yet about raw vs reduced.'),
    ('chandra',                 'Chandra X-ray Observatory',          'tap',       FALSE, NULL, 'X-ray grating spectroscopy, not the optical/IR regime every other archive here covers -- a deliberate scope extension to cover X-ray spectra alongside optical/IR. Implemented via a real TAP service at cda.harvard.edu/cxctap (found via the reg.g-vo.org registry sweep), table cxc.observation -- CDA''s own observation-log schema, not ivoa.obscore. Filtered to grating IN (''HETG'',''LETG'') to isolate real dispersed-spectrum exposures from Chandra''s much larger imaging-only holdings, and status IN (''archived'',''observed'') to exclude scheduled-but-not-yet-taken rows. 3,243 real rows observed, 0 masked ra/dec. No access_url/ObsCore column on this table -- archive_url points at the real per-observation archive browser page (chaser/startViewer.do) instead of a direct FITS link. reduction_status left unset -- no calib_level-equivalent column exists here.'),
    ('irtf_spex',               'IRTF SpeX (via IRSA)',              'tap',       FALSE, NULL, 'Implemented via IRSA''s TAP service (irsa.ipac.caltech.edu/TAP), joining caom.observation_irtf + caom.plane_irtf + caom.artifact_irtf (a CAOM2 model split the same way as CADC''s) -- covers only the 2016B-present IRSA-hosted portion of IRTF''s holdings; the pre-2016B IRTF Legacy Archive (irtfdata.ifa.hawaii.edu) is pure HTML directory browsing with no API found, not covered. 86,511 SpeX rows observed, all calibrationlevel=1 (raw) -- no reduced-product tier exists in this table to pick between. No position data at all (observed: every row''s targetposition_coordinates is NULL, no s_ra/s_dec-equivalent column exists either) -- name-only match, target_name cleaned (underscore-to-space, strips an appended "_AV=..." reddening suffix on ~750 rows) before matching. archive_url points at each observation''s real summary.html landing page, not a raw FITS artifact (each plane bundles several raw frames -- flats/arcs/target -- with no single canonical file). Pagination watermarked on plane.time_bounds_lower (MJD), with a client-side same-timestamp planeid-dedup guard -- observed that IRSA''s TAP output rounds this column to 6 decimals for display but compares full precision server-side, so a naive watermark can re-match its own boundary row on the next page. Shares fetch logic with irtf_ishell.py (see sync/archives/_irtf_common.py) -- same tables, only instrument_name differs.'),
    ('irtf_ishell',             'IRTF iSHELL (via IRSA)',            'tap',       FALSE, NULL, 'Same IRSA TAP tables/module as irtf_spex (sync/archives/_irtf_common.py) -- iSHELL was missed in the first investigation pass (Wikipedia''s IRTF page lists it alongside SpeX). 28,126 rows observed, same shape as SpeX in every respect checked: all calibrationlevel=1 (raw), one info/text/html summary.html artifact per plane (1:1, observed), same underscore-joined target_name convention. No position data, name-only match, same as SpeX.'),
    ('irtf_legacy',             'IRTF Legacy Archive',               'html_form', FALSE, NULL, 'The pre-2016B piece of IRTF''s holdings (irtf_spex/irtf_ishell cover 2016B-present via IRSA) -- previously written off after only checking the directory-browser page; the /search/ page turned out to be a real, unauthenticated GET form (results.php) returning a structured HTML table with per-frame ra/dec, target name, program ID, and a direct file path, observed. Covers sbd_1/sbd_2 (SpeX''s two hardware generations across this era, 2000-2016A) and cshell (CSHELL, IRTF''s other retired high-res NIR spectrograph, observed to have real substantial volume -- 1,190 of a 5,000-row sample) -- mirsi_1 (dual imaging/grism-spectroscopy mode, no column to tell which) deliberately excluded, same reasoning noirlab.py/ing.py give for their own dual-mode exclusions. Row granularity is per-raw-FITS-file (observed, e.g. each of a sequence''s own arc/flat calibration frames is its own row, object name literally "Argon lamp"/"Inc lamp") -- no plane/block id exists in this table to group on, and no calibration-frame filter is attempted (same reasoning as lick.py/ing.py: those names simply never match a tracked star and fall through to a harmless skip). Hard-capped at EndUTCDate < 2016-08-01 (exclusive) to avoid the same physical observations appearing under both this archive_code and irtf_spex/irtf_ishell''s. Paginated via an ing.py-style adaptive UTC-date-window crawl against the form''s own confirmed-live 5000-row cap.'),
    ('lco_floyds',              'Las Cumbres Observatory -- FLOYDS',  'rest_json', FALSE, NULL, 'Implemented via the public archive-api.lco.global REST API (unauthenticated, no key needed for public data) -- covers FLOYDS (low-res long-slit, en-coded instruments), observed to be the only spectrograph behind OBSTYPE=SPECTRUM (52,038 public frames). Groups frames by observation_id (BLKUID) and picks the single best-available reduction tier per block (a real basename ending "-1d", else RLEVEL=90, else raw) rather than filtering to one fixed RLEVEL -- observed that most real observations only ever get a raw frame archived (BANZAI-FLOYDS reprocessing lags/never completes for most exposures), so a fixed-RLEVEL filter would have silently dropped the majority of real data. Internal engineering/commissioning frames (proposal_id==''calibrate'' exactly, observed to include a shared bogus 1973 placeholder date and legacy IRAF-stage intermediate RLEVELs like 14/24/35/67/98) are excluded client-side; fetch() loops internally past an all-noise page so this filtering can never look like "caught up" to the runner. Position data is per-frame, not fixed per-instrument (observed: raw/RLEVEL-90 frames carry a real sky-footprint polygon, the final "-1d" product does not) -- ra/dec set from whichever tier was actually picked. archive_url is the API''s own stable per-frame resolver (frames/{id}/), not the embedded `url` field, which is a pre-signed S3 link that expires after 48 hours. Anonymous requests cap page size at exactly 100 (observed: 150+ -> HTTP 400) -- watermarked on observation_date, with a client-side same-timestamp id-dedup guard since the API''s own `start=` filter is inclusive and has no working strict-inequality lookup (observed). Shares grouping/pagination logic with lco_nres.py (see sync/archives/_lco_common.py).'),
    ('lco_nres',                'Las Cumbres Observatory -- NRES',    'rest_json', FALSE, NULL, 'LCO''s second spectrograph (Network of Robotic Echelle Spectrographs, high-res, fiber-fed) -- missed in the first investigation pass, which only found FLOYDS; LCO''s own instruments page lists both. Same shared module as lco_floyds.py (sync/archives/_lco_common.py), identified by OBSTYPE=TARGET rather than SPECTRUM (observed -- TARGET also covers plain Sinistro imaging sharing the same fa-prefixed instrument codes, so OBSTYPE is the real discriminator, not instrument_id). Real reduced fraction is much lower than FLOYDS''s: only 1,034 of 73,200 raw TARGET frames observed to have reached a BANZAI-NRES product at all -- the same per-block best-tier-or-raw-fallback grouping as FLOYDS is what keeps this from dropping ~98% of real NRES observations. Per LCO''s own instruments page, NRES "will not be offered as of semester 2026B" -- winding down, but its historical public spectra stay in scope. Unlike FLOYDS, has real position data on many frames (GeoJSON sky-footprint polygon, observed) -- ra/dec set from the polygon centroid when present, giving a genuine (if imprecise -- observed footprints can be ~10 arcsec across, wider than matcher.py''s default 1" easy-match radius) positional fallback behind identifier matching.'),
    ('ondrejov',                'Ondrejov Observatory (CCD700)',      'tap',       FALSE, NULL, 'Implemented via a real GAVO DaCHS/SSA TAP service at voarchive.asu.cas.cz/tap (table ccd700.data), same software/shape as feros_gavo/hermes_mercator -- observed 22,325 real spectra (mostly Be stars/emission-line objects) from the 700mm coude camera at the Ondrejov 2m Perek Telescope, Czech Republic, R~13,000 around Halpha (twice that near Hbeta) per the service''s own info page. Same paired application/x-votable+xml metadata-row quirk as feros_gavo/flashheros_gavo (filtered via mime), but unlike those two, every real row has a populated ssa_targname and ssa_location -- normal identifier-then-position matching applies. ssa_location is a plain "Position ICRS <ra> <dec>" text string here, not an array. Still actively growing (service reports "Data updated" today, ssa_dateobs reaches into 2026) -- paginated by an ssa_dateobs watermark, same t_min-style shape as eso.py/dao.py.'),
    ('heros_ondrejov',          'HEROS at Ondrejov',                  'tap',       FALSE, NULL, 'A third, distinct HEROS-adjacent archive: not flashheros_gavo.py (Heidelberg''s own late-1990s La Silla holdings) and not ondrejov.py (Ondrejov''s separate CCD700 coude spectrograph) -- this is the Ondrejov-owned share of HEROS itself, run for a few years on Ondrejov''s own Zeiss 2m telescope under a joint-but-separate agreement with Heidelberg, per a collaborator tip. Found at vos2.asu.cas.cz/heros/q/web/form, same GAVO DaCHS software family as flashheros_gavo/ondrejov/feros_gavo/hermes_mercator, so the same shortcut applies: a real TAP endpoint at vos2.asu.cas.cz/tap (table heros.data). Observed via ssa_creator="Heros Ond"/ssa_instrument="Ondrejov Zeiss 2m" -- genuinely the Ondrejov-mounted instrument, not a mirror of Heidelberg''s data. 1,010 real spectra (mime=''image/fits'', same paired-row quirk as its GAVO siblings), MJD 51770-52727 (2000-09-25 to 2003-04-15) -- HEROS hasn''t operated at Ondrejov since. Unlike flashheros_gavo.py, ssa_location is populated on every row -- normal identifier-then-position matching, same "Position ICRS <ra> <dec>" text format and parser as ondrejov.py. No plain instrument column on this table (observed) -- hardcoded to "HEROS (Ondrejov)".'),
    ('polarbase',               'PolarBase (ESPaDOnS/Narval/SPIRou/HARPSpol spectropolarimetry)', 'rest_json', FALSE, NULL, 'Petit et al. 2014 PASP database of reduced Stokes-parameter spectropolarimetric products -- a different, additive data product from cfht_cadc''s raw ESPaDOnS ObsCore rows, not a duplicate. The registered SSA service (ivo://ov-gso/ssap/polarbase at polarbase.ovgso.fr and the older polarbase.irap.omp.eu domain both observed, same backend IP, same VOTable error response) turned out to be a dead end for a bulk pull -- POS/SIZE cone-search only, SIZE capped at 5 deg, no full-archive mode. The real access path is an undocumented JSON REST API found in the SPA''s own JS bundle (/api/spectra, documented at /api/docs/openapi.yaml once found) -- observed to hard-cap at 10,000 records/response with no ORDER BY control at all, and observed that even single-calendar-year windows (2007-2024) each independently hit that exact cap, so this paginates via an adaptive calendar-window walk instead of a date watermark, same undocumented-row-cap shape as ing.py/gtc.py (bisects the window on a capped response, grows it back up once under cap). Observed to cover five real instruments, not just ESPaDOnS/Narval as expected going in: espadons (CFHT), narval + neo_narval (TBL, Pic du Midi, not covered by any other archive here), spirou (CFHT, near-IR), and harpspol (ESO 3.6m/HARPS polarimetric mode). A full walk from 2000-01-01 to present observed 346,273 distinct real spectra -- far more than a quick non-exhaustive sample suggested, exactly the undercount the calendar-window walk exists to avoid. The API''s own join has a real duplicate-row artifact -- some (id_observation, stokes) pairs come back as two byte-for-byte identical records in the same response -- deduped client-side by id_observation alone. Real ra/dec (decimal degrees) and target name on every sampled row -- normal identifier-then-position matching, not name-only.'),
    ('subaru_moircs',           'NAOJ (Subaru MOIRCS, via JVO)',      'tap',       FALSE, NULL, 'A second Subaru instrument on the same JVO engine as naoj.py''s HDS endpoint -- naoj.py''s own docstring wrote MOIRCS off as imaging-only based only on a registry SSA/SIA capability check; that was wrong. Real TAP endpoint (jvo.nao.ac.jp/skynode/do/tap/moircs, table public.raw) observed, filtered to obs_mode IN (''SPEC'', ''SPEC_MOS'') to isolate 36,604 real dispersed-spectrum rows from 89,858 imaging rows (obs_mode=IMAG) and other non-science modes. Same JVOQL quirks as HDS -- COUNT(DISTINCT ...) silently ignored (observed), FORMAT=csv ignored, 200,000-row RECORD_MAX cap -- but SELECT * fails for a different reason here (`column t0.center_ra does not exist`, observed, not HDS''s malformed access_estsize column). Unlike HDS, no per-exposure pipeline-product dedup needed -- every row is a genuinely distinct FITS frame from one of MOIRCS'' two separate detector chips (observed via a live GROUP BY data_id HAVING COUNT(*) > 1 returning zero rows). ref_val1/ref_val2 observed to be plain FK5 J2000 RA/Dec in degrees, not a WCS reference needing conversion. Paginated via an id integer watermark, no cliff found (whole 36,604-row set fetched in one ~6.4s query).'),
    ('iacob',                   'IACOB Spectroscopic Database (IAC)', 'ssa',       FALSE, NULL, 'A curated OB-star spectroscopic database at the Instituto de Astrofisica de Canarias -- overlaps in part with hermes_mercator.py''s own Mercator/HERMES holdings (a different door into some of the same underlying observations, not deduplicated against it, same as other overlapping archives in this project). No plain TAP endpoint on this host (observed: /tap, /iacob/tap, and the DaCHS __system__/tap/run/tap convention all 404 with a bare Tomcat page) -- implemented against a bespoke JSP SSA service (ocan.iac.es:8080/iacob/jsp/ssap.jsp) instead. No sky-crawl needed: POS=0,0 with SIZE=360 (a full-sky cone search) returns a stable, convergent 1,255-row whole-archive total in one request (observed at multiple SIZE values approaching and exceeding 180 degrees). The response''s TIME field is declared an invalid VOTable datatype (TIMESTAMP, observed -- astropy/pyvo reject the whole response outright), so it''s parsed via a plain regex TR/TD walk instead of astropy''s VOTable parser. SSA''s TIME range filter errors out on this service (observed) -- no incremental watermark is available, so this does one full pull and no-ops thereafter via a synced_at cursor, same shape as feros_gavo.py/elodie.py, though unlike those this is a documented current limitation (Mercator/NOT are still operating telescopes) rather than a genuinely finished dataset. Only two instruments appear in the live rows: MERCATOR (362 rows, R 85,000) and NOT (893 rows, R 25,000-67,000 depending on FIES mode).'),
    ('ritter_prest',            'Ritter Observatory (PREST)',        'html_form', FALSE, NULL, 'University of Toledo''s 1.06-m Ritter Observatory, NSF PREST program -- a personal faculty page (astro1.panet.utoledo.edu/~wwritter/archive/), not an institutional archive endpoint, so this is a real permanence risk (no known mirror) unlike every other archive in this table. No TAP/SSA/API and no directory listing (FITS-spectra/ 403s) -- scraped from two static index pages (HDsearch.html, PREST-archive.html) that union to 92 star/group page links, 4 of which 404 (observed dead/typo''d hrefs). Solar-system objects (Jupiter, Callisto, Ganymede, Io, Mars, Moon, Venus, two comets) and a synthetic telluric-template page are excluded by directory name -- stars only. Per-star HTML pages carry no ra/dec/date at all -- pulled instead from each FITS header via an HTTP Range GET (first 4 of ~17-18 2880-byte blocks, real headers end within 2, observed), keyed on the header''s own OBJECT field rather than the (sometimes multi-star) page grouping label. Observed via a real fetch({}): 2,218 candidate stellar FITS links resolve to 2,201 ingested records (17 individually 404 despite being linked from a live page) across ~175 named stars, 1994-2007 observations, reprocessed 2011-2016 -- frozen/historical, one-shot pull via a synced_at cursor like elodie.py/iacob.py; a second fetch() with the returned cursor observed to be an instant no-op. All spectra reduced (bias-subtracted, flat-fielded, extracted, wavelength-calibrated) per the archive''s own description.'),
    ('svo_cab',                 'SVO CAB Stellar Libraries',         'ssa',       FALSE, NULL, 'Five small curated empirical stellar libraries hosted on one shared SVOCat SSA stack at svo2.cab.inta-csic.es/vocats (same one-archive-many-instruments shape as oirsa.py): MILES (985 stars), STELIB (256), XSL/X-Shooter Spectral Library (912), CaT/Ca II Triplet library (696), and Gaia FGK Benchmark Stars (241, note the different "vocats/gbs/..." path shape vs the other four''s "vocats/v{2,3}/<name>/..."). These are SSA cone-search services, not TAP, but each one''s own search-form help text advertises "Maximum Search Radius allowed: 180 degrees" as a genuine radius (observed: identical ~985-row MILES result from two different POS centers at SIZE=180) -- so one SIZE=180 query per library pulls its entire static catalog in a single page, no sky-grid crawl needed. Column names are not uniform across the five (independently configured SVOCat instances) -- per-collection field map in sync/archives/svo_cab.py. Only XSL carries a real per-row observation date (Epoch, MJD; populated on 245 of 912 rows) -- the other four are one-shot reference compilations with no date field at all. Final, static datasets, one-shot pull like rave.py/feros_gavo.py.'),
    ('irsa_missions',           'IRSA Space-Mission Stellar Collections', 'ssa',   FALSE, NULL, 'Six independent historical space/airborne-mission stellar spectral collections behind IRSA''s shared SSA service (irsa.ipac.caltech.edu/SSA?COLLECTION=...), same one-archive-many-instruments shape as oirsa.py/svo_cab.py: Spitzer Atlas of Stellar Spectra (spitzer_sass, 159 stars), Spitzer IRS Standard Stars (spitzer_irs_std, 73), ISO/SWS, IRAS/LRS Atlas, SOFIA/EXES (sofia_exes, 2,580 distinct observations across 29,212 raw per-order/per-nod files -- SOFIA retired 2022, fully archived), and IRTF/MEarth (irtf_mearth, 468 M dwarfs, 498 spectra) -- a small standalone IRSA table distinct from the IRTF CAOM2 TAP holdings already covered by irtf_spex.py/irtf_ishell.py/irtf_legacy.py (flagged as a known gap in irtf_spex.py''s own docstring, closed here). Also includes three stellar Spitzer/IRS legacy-survey tables served via IRSA''s TAP service instead of SSA (added 2026-09-18): FEPS (328 solar-type stars), Disks SH (54 disk-hosting stars) and c2d (1,382 young-stellar-object spectra) -- undated, IPAC .tbl/.dat files. Deliberately excludes IRSA''s other Spitzer collections found alongside these (spitzer_sings, spitzer_m83m33, spitzer_sage, spitzer_s5, spitzer_5muses, spitzer_ssgss, spitzer_goals) -- confirmed extragalactic/ISM surveys, not stellar. SIZE behaves as a real radius here too (observed the same way as svo_cab.py: identical 498-row irtf_mearth result from opposite-hemisphere POS centers at SIZE=180) -- spitzer_sass/spitzer_irs_std/sofia_exes/irtf_mearth each pull their whole catalog in one SIZE=180 query. iso_sws and iras_lrs both hard-timeout server-side at that SIZE ("Job ran but timed out", observed) despite being modest collections -- paginated instead via a justified 17-cell sky-grid crawl (SIZE=45/cell, cos(dec)-scaled RA spacing), one (collection, cell) pair per fetch() call, same one-window-per-call shape as ing.py.'),
    ('xmm',                     'XMM-Newton RGS',                    'tap',       FALSE, NULL, 'X-ray grating spectroscopy, same regime as chandra.py -- implemented via a real TAP service at nxsa.esac.esa.int/tap-server/tap (ESA''s XSA), joining xsa.v_exposure to xsa.v_public_observations on observation_id (both char, observed). Filtered to instrument LIKE ''RGS%'' AND mode_friendly_name LIKE ''Spectroscopy%'' AND is_scientific=''true'' to isolate real dispersed-spectrum exposures from RGS''s other readout modes (Diagnostic/HTR) and from XMM''s much larger EPIC/OM holdings -- 33,819 real rows observed, 0 masked ra/dec, 42 rows with a blank target name. RGS1 and RGS2 kept as two separate holdings (real, physically distinct simultaneous gratings, same reasoning as Chandra''s HETG/LETG) -- their exposure_id is not unique per observation on its own (observed: RGS1 and RGS2 can share the same exposure_id), so archive_obs_id keys on observation_id+instrument+exposure_id together. A single unbounded TOP 50000 pull (real headroom over the confirmed total) returns in ~3s, avoiding a windowed-pagination boundary landing on one of the 5,116 start_utc timestamps shared by both gratings'' simultaneous exposures. proprietary_end_date is a real, populated embargo field (observed future dates) but not filtered on, same as salt_hrs.py -- embargoed rows still answer whether a star has been observed at all. archive_url points at nxsa-web/#obsid=... -- confirmed real via the archive''s own compiled GWT JS bundle, which literally parses an ''obsid='' history token, not a guessed URL. reduction_status left unset -- no calib_level-equivalent column exists on either table.'),
    ('vizier_assocdata',        'VizieR Associated Data (CDS)',       'tap',       FALSE, NULL, 'Found via cdsarc.cds.unistra.fr/assocdata -- a real ObsTAP/ADQL service (Saada engine, endpoint cdsarc.cds.unistra.fr/saadavizier.tap/tap, table obscore) aggregating spectra individual journal articles published as VizieR electronic tables, one obs_collection per publication (284 total carry dataproduct_type=''spectrum'', 8.3M rows observed 2026-08-24). Ranking rule: always prefer a direct archive link; this is the fallback only. Most big buckets duplicate archives already synced directly (LAMOST 7.67M rows via lamost.py/lamost_mrs.py, HST via mast.py, AAT''s J/MNRAS/413/971 is literally RAVE via rave.py, plus ESO/VLT/NTT, Keck, Gemini, TNG, Mercator, CFHT, OHP, SALT, GTC, SDSS, ING/NOT telescopes, Subaru, IRTF, Lick, NOIRLab-operated CTIO/KPNO/SOAR/Blanco/Bok, Spitzer, Ritter, LBT, Asiago, FUSE, JWST, XMM) -- excluded via an exact, hand-verified facility_name literal list (EXCLUDED_FACILITY_NAMES in sync/archives/vizier_assocdata.py; ADQL here has no LOWER()/LIKE support so this is exact-match, not substring). Also restricted to em_min/em_max in [1e-7, 5e-6] m (100nm-5000nm) to drop the radio/mm long tail (IRAM/JCMT/ALMA/VLA/Effelsberg/ATCA/MeerKAT/VLBA/NOEMA/Arecibo/...) this project has no archive for. Two facility_name buckets that looked like real spectra by count/wavelength alone turned out not to be real stellar observations and are excluded too: COROT (B/corot, 177,553 rows) is mislabeled CoRoT photometric time-series flux -- most rows have no target_name, but the COROT_faint_star channel does carry one (a non-SIMBAD-resolvable internal run id, e.g. "COROT105288043"), so target_name-non-null alone was not a sufficient filter; and Model (VII/102, 96 rows) is synthetic spectral-type template spectra (target_name = "F56V"/"G04V"/...), not observations of real stars. 13,199 rows remain after all filters (confirmed live 2026-08-24) -- small single-paper collections from telescopes/PIs with no ongoing public archive of their own, reachable here only because the journal-supplement copy is public on CDS (same category of PI-only data written off as unreachable for Palomar/APO). calib_level is -1 (unset sentinel) on every remaining row -- reduction_status left ''unknown''. Paginated via an oidsaada id watermark (a real per-row unique identifier, ~19-digit, string-comparison-safe since same length across the observed range) since VizieR keeps ingesting new journal tables over time, not a frozen dump -- same shape as hermes_mercator.py/asiago.py. bib_reference (the paper''s bibcode) stored in program_id for provenance, not a literal observing-program ID.'),
    ('spitzer_sha',             'Spitzer Heritage Archive (IRS + MIPS-SED)', 'rest_json', FALSE, NULL, 'Per-AOR metadata for every IRS staring/mapping observation plus MIPS-SED (55-95 um), Spitzer''s 2003-2009 cryogenic-mission spectra that irsa_missions.py''s small curated collections (SASS, IRS standard stars, FEPS, Disks SH, c2d) miss -- e.g. TW Cam''s 2004 IRS Stare. No documented API: found 2026-09-18 by replaying the SHA web app''s own Firefly search (unauthenticated POST to sha.ipac.caltech.edu/applications/Spitzer/SHA/CmdSrv/sync?cmd=tableSearch, id aorByPosition), crawled as a fixed sky grid since whole-sky queries time out. Real observation dates on every row. archive_url is the AOR''s directory on IRSA''s file server (per-exposure raw/bcd/pbcd products, not one combined 1D spectrum). See sync/archives/spitzer_sha.py.');
