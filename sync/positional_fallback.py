...
def run_shitty_positional_match(conn: psycopg.Connection, archive_code: str, records: list[RawObservation]) -> dict:
    ...
    # observation decade rather than one flat max_years for the whole chunk.
    # A single worst-case radius forces every record to pay for whichever
    # one observation in the chunk happens to be oldest -- confirmed live:
    # dao's own backlog spans 1986-2026, padding the search radius to ~369"
    # (6.1', ~36x the unpadded area) for every record in a chunk even though
    # most of them are far more recent and need much less. Bucketing by
    # decade bounds the extra Gaia round-trips per chunk (at most a handful
    # of decades) while sizing each bucket's own padding to its own actual
    # date range.
    decade_buckets: dict[int, list[int]] = defaultdict(list)
    for i, r in enumerate(positional):
        decade_buckets[(r.obs_date.year // GAIA_QUERY_TIME_BUCKET_YEARS) * GAIA_QUERY_TIME_BUCKET_YEARS].append(i)

    live_hits_raw: dict[int, list[tuple]] = {}
    for idxs in decade_buckets.values():
